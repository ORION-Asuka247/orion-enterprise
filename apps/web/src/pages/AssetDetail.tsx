...
export default function AssetDetail() {
  ...
  const location = [asset.properties?.name, asset.blocks?.name, asset.floors?.name, asset.areas?.name].filter(Boolean).join(" - ");
  const activeDefects = defects.filter(d => !["resolved", "verified", "closed"].includes(d.status)).length;

  return (
    <Page title={asset.asset_code} kicker="PERMANENT ASSET RECORD">
      <div className="page-toolbar">
        <div><p className="muted">{asset.name || asset.asset_types?.name || "ORION asset"}</p></div>
        <div className="toolbar-actions">
          <Link className="button-link" to={`/assets/${asset.id}/inspect`}>Start inspection</Link>
      ...
      <div className="asset-detail-grid">
        ...
        <aside className="panel qr-card">
          ...
          <div className="qr-code-wrap">
            <QRCodeSVG value={qrUrl} size={220} level="M" bgColor="#ffffff" fgColor="#000000" title={`ORION ${asset.asset_code}`} />
          </div>
      ...
      <div className="metrics asset-history-metrics">
        ...
        <div className="metric"><span>Open defects</span><strong>{activeDefects}</strong><small>Requires attention</small></div>
        <div className="metric"><span>Lifecycle events</span><strong>{statusHistory.length}</strong><small>Status history</small></div>
      </div>

      <section className="panel">
        <div className="section-heading"><div><div className="eyebrow">AUDITABLE HISTORY</div><h2>Asset timeline</h2></div></div>
        {timeline.length === 0 ? (
          ...
          <div className="asset-timeline">
            {timeline.map(item => (
              <article className="timeline-item" key={item.key}>
                ...
                <div>
                  <div className="timeline-meta"><span>{item.category}</span><time>{displayDate(item.date)}</time></div>
